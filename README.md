...
1. **IoT Device Simulators**: Simulate data from IoT devices that send data every second.

2. **Kafka Message Broker**: Collects data from devices and distributes it to processors.

3. **Data Processor (Spring Boot Service)**: Consumes data from Kafka topics, processes it, and stores it in MongoDB.

4. **API Server (Spring Boot)**: Exposes secure REST APIs for querying sensor data (average, min, max).

5. **MongoDB Database**: Stores raw sensor data and aggregated results.

6. **Query Engine**: Computes aggregated values like average, max, min.

### High-Level Block Diagram
# IoT Data Collection and Processing Pipeline
